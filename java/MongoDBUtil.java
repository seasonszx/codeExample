...
public enum MongoDBUtil {
	...
	static {
		...
		// List<ServerAddress> listHost = Arrays.asList(new ServerAddress("localhost",
		// 27017),new ServerAddress("localhost", 27018));
	...
	public void insertDocument(MongoCollection<Document> coll,Document doc) {
		coll.insertOne(doc);
	}
	...
	public static void main(String[] args) {
		...
		String dbName = "zanadb";
		String collName = "loggs";
		MongoCollection<Document> coll = MongoDBUtil.instance.getCollection(dbName, collName);
		// 测试1
		// MongoDatabase mdb = MongoDBUtil.instance.getDB(dbName);
		// System.out.println("数据库名称："+mdb.getName());
		// 测试2
		// List<String> l= MongoDBUtil.instance.getAllCollections(dbName);
		// for (String string : l) {
		...
		// //查询总记录
		// int
		// cout=MongoDBUtil.instance.getCount(MongoDBUtil.instance.getCollection(dbName,
		// collName));
		// System.out.println(cout+"总数");

		// 插入多条
		for (int i = 1; i <= 4; i++) {
			Document doc = new Document();
			doc.put("name", "zhangsan"+i);
			doc.put("school", "njnydx" + i);
			Document interests = new Document();
			interests.put("game", "game" + i);
			interests.put("ball", "ball" + i);
			doc.put("interests", interests);
			MongoDBUtil.instance.insertDocument(coll,doc);
			coll.insertOne(doc);
		}
